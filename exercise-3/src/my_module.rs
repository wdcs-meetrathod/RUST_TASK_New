...
pub fn cache_price(time: u32) -> Result<f64, String> {
    ...
    let (mut socket, _response) =
        connect(Url::parse(&binance_url).unwrap()).expect("Could't connect to websocket");
...
pub fn read_price() {
    let contents =
        fs::read_to_string("./priceDetails.txt").expect("Should have been able to read the file");
...
pub fn get_keys(client: i32) -> Option<ClientKeys> {
    let client_keys: Vec<ClientKeys> = vec![
        ClientKeys {
            client: 1,
            private_key: "d367b3e44029bcb967463a030f4d7912f754fdf3df7e95094faf7ae20bfbb1d6"
                .to_string(),
            public_key: "5dd304258e3bcc9d43a1773ecfac51028ce180028502c4d2929adc9a9daa9b57"
                .to_string(),
        },
        ClientKeys {
            client: 2,
            private_key: "60aeb5c892f8983def0dce3dceb902af32bd47264e5f76bb0c48cc723de8afbc"
                .to_string(),
            public_key: "2e857830d934b1d43dffcf8c97f7790045c77518a3b405244e84a2e9199f0997"
                .to_string(),
        },
        ClientKeys {
            client: 3,
            private_key: "9983fffa551801060638fb4ea8968fa879768d1d9cd8d8c22f9a53d6581253c7"
                .to_string(),
            public_key: "9831cdf3d299de2eea35140ade28c9fc6e0858837affc5d56317208e2deb30c4"
                .to_string(),
        },
        ClientKeys {
            client: 4,
            private_key: "e3a1e9af3dce9ada31a4b0ddbc31818909efa6081af52dc112d0db87691c750f"
                .to_string(),
            public_key: "417f173230605ba5e89a98bad1a04a3d3ca6ba25b68423b8ffd0e431d738db0c"
                .to_string(),
        },
        ClientKeys {
            client: 5,
            private_key: "970fefe9139a781d7bad9954bcdc1ac841aa52309dee65b874491b01561bafdc"
                .to_string(),
            public_key: "7bdfe218f62e5d472c1d5192966896a374d0a81cd4521d0fed6a5f0e930a0800"
                .to_string(),
        },
    ];
...
fn get_key_pair(client: i32) -> Keypair {
    if let Some(client_key) = get_keys(client) {
        let private_key_bytes =
            hex::decode(&client_key.private_key).expect("Invalid private key hex string");

        let public_key_bytes =
            hex::decode(&client_key.public_key).expect("Invalid public key hex string");
...
pub fn verify_message(client: i32, (message, signature): (String, &[u8])) -> Option<String> {
    let key_pair = get_key_pair(client);
    ...
    if auth.is_ok() {
        ...
        eprintln!("Invalid signature from client {}", client);
        None
    }
}
